...
public class LoggerPrinter implements Printer {
     ...
     * so 4000 bytes as chunk size since default charset
     * is UTF-8
     */
     ...
     */
    private static final int JSON_INDEX = 4;

    /**
     * The minimum stack track index, starts at this class after two native calls.
     */
    ...
    @Override
    public void json(String json, int logType) {
        String message = null;

        if(TextUtils.isEmpty(json)) {
            relayLogByType("Empty/Null json content", logType);
            return;
        }
        try {
            json = json.trim();
            if(json.startsWith("{")) {
                ...
                message = jsonObject.toString(JSON_INDEX);

            }
            if(json.startsWith("[")) {
                JSONArray jsonArray = new JSONArray(json);
                message = jsonArray.toString(JSON_INDEX);
            }
            relayLogByType(message, logType);
    ...
    @Override
    public void xml(String xml, int logType) {
        ...
        try {
            ...
            relayLogByType(xmlOutput.getWriter().toString().replaceFirst(">", ">\n"), logType);
    ...
    /**
     * relay log by type and appropriate to json or xml log
     * @param message
     * @param logType
     */
    private void relayLogByType(String message, int logType) {
        log(logType, message);
    }
    ...
    /**
     * This method is synchronized in order to avoid messy
     * of log`s order.
     *
     * @param logType
     * @param msg
    ...
    private synchronized void log(int logType, String msg, Object... args) {
        ...
        //Get bytes of message with system`s
    ...
    private void logHeaderContent(int logType, String tag, int methodCount) {
        ...
        if(settings.isShowThreadInfo()) {
            logChunk(logType, tag, VERTICAL_DOUBLE_LINE + " Thread: " + Thread.currentThread().getName());
            logDivider(logType, tag);
        }
        ...
        //corresponding method count width the current stack may exceeds the
